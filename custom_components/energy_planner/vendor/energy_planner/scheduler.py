...
def run_once(now: Optional[datetime] = None) -> dict:
	...
	if settings.use_linear_solver:
		try:
			from .optimizer.linear_solver import solve_optimization_linear
			result = solve_optimization_linear(forecast_frame, context)
		except ImportError:
			print("Linear solver not available, falling back to default solver.")
